...
interface HomeClientProps {
  ...
  preloadedSubmission: Preloaded<typeof api.submissions.get> | null;
}

// Wrapper component that handles preloaded data
function HomeClientWithPreload({
  submissionId,
  sessionId,
  preloadedSubmission,
}: {
  submissionId: Id<"submissions">; // non-null here
  sessionId: string | null;
  ...
  const submission =
    activeSubmissionId == null
      ? undefined // after reset: behave as if there's no submission
      : activeSubmissionId === submissionId
        ? preloaded // initial id → use SSR preloaded data + live updates
        : liveSubmission; // new id → use the live query
  ...
  return (
    <HomeClientContent
      ...
      onSubmissionChange={setActiveSubmissionId}
    />
  );
}

// Wrapper component that fetches data via query
function HomeClientWithQuery({
...
// Main export that chooses which wrapper to use
export default function HomeClient({ submissionId, sessionId, preloadedSubmission }: HomeClientProps) {
  if (preloadedSubmission && submissionId) {
    return (
      <HomeClientWithPreload
  ...
  return <HomeClientWithQuery submissionId={submissionId} sessionId={sessionId} />;
}

// The actual content component
type Submission = ReturnType<typeof usePreloadedQuery<typeof api.submissions.get>> | undefined;
...
function HomeClientContent({
  ...
  sessionId: string | null;
  submission: Submission;
  onSubmissionChange?: (id: Id<"submissions"> | null) => void;
}) {
  ...
  const handleGrade = async () => {
    ...
    if (response?.data?.submissionId) {
      ...
      onSubmissionChange?.(response.data.submissionId);
      // Use history.replaceState to update URL without triggering Next.js re-render
      // This prevents component remounting which would reset isSubmitting state
      window.history.replaceState(null, "", `/?submission=${response.data.submissionId}`);
    } else {
  ...
  }, [submission?.status, submission?.result, submission?.error, submission?.text, router, toast, text]);

  const handleReset = () => {
    setText("");
    setResult(null);
    // Clear the active submission ID to stop the query
    onSubmissionChange?.(null);
    // Use history.replaceState to avoid triggering Next.js re-render
